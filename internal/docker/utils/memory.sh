...
compute_memory() {
	...
	# OS gets 20-30% of memory, minus reserved code cache size
	if [ -z ${OS_MEM} ]; then
		OS_MEM_AND_CODE_CACHE_MB=$((${SYSTEM_TOTAL_MEMORY_MB} / 100 * ${OS_MEM_PERCENT}))
		export OS_MEM=$((${OS_MEM_AND_CODE_CACHE_MB} - ${RESERVED_CODE_CACHE_SIZE}))
	fi

	# Core gets one third of remaining memory
	# The ceiling for Core is 8GB, it doesn't get more than that
	if [ -z ${CLOVER_SERVER_HEAP_SIZE} ]; then
		MEMORY_REMAINING=$((${SYSTEM_TOTAL_MEMORY_MB} - ${OS_MEM} - ${RESERVED_CODE_CACHE_SIZE}))
		export CLOVER_SERVER_HEAP_SIZE=$((${MEMORY_REMAINING} / 3 > 8192 ? 8192 : ${MEMORY_REMAINING} / 3))
	fi
